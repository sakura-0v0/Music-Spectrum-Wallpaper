...
- **推荐的几个配置组合：（始终启用“锁定窗口”）**
1. 什么都不勾选（位于图标上方，所有其他窗口下方）
2. 壁纸模式 + 置顶窗口（位于其他壁纸软件上方，桌面图标下方）
3. 置顶窗口（像歌词一样）
...
'''
  ...
  ### 注意：一定要安装PyAudio-0.2.11-cp37-cp37m-win_amd64.whl，不能直接xx pip install PyAudio xx
...
**注意**：首次运行时会自动生成配置文件，建议先通过配置界面调整适合自己系统的参数。
